...
20. **If a remargin result spills to a file, call `report_spill` BEFORE you read that file.** When a remargin tool result exceeds your client's output-token limit, the client spills it to a scratchpad file and tells you to read the whole thing. The moment that happens, call `report_spill` first — it lets remargin infer the offending size from its last result and ratchet its per-session page cap down so future `search` pages stay under your limit. The cap only falls, never rises, and resets each session, so one report keeps you from spilling again. Pass `size` only to override the inferred value. A `search` response carrying `effective_limit` is already a bounded page — use its `total` and `offset` to page for the rest rather than forcing a spill.
...
| `search` | Search across documents for text. `regex`, `scope` (all/body/comments), `context`, `ignore_case`, `limit`/`offset` (paged; response always carries `total`). Pages are auto-sized under the session spill cap; a bounded page carries `effective_limit` — page via `offset` for the rest. |
| `report_spill` | Signal that your client just spilled a remargin result to a file (over its output-token limit). Ratchets the session's page cap DOWN so future `search` pages fit. Infers the size from the last result; `size` overrides. Call it BEFORE reading the spilled file. See Critical rule 20. |
