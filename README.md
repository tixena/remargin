...
The command is careful with the file as a whole. goose keeps its provider, its model, and every other extension in that same `config.yaml`, and a config goose cannot parse costs the user every session rather than just remargin's tools — so install and uninstall touch exactly remargin's entry, refuse to overwrite a config that does not parse, and write through a temp file and a rename. Reinstalling an already-canonical entry leaves the file untouched. A layout the in-place editor does not model — a flow-style `extensions: {...}` value, say — is written by re-serializing the whole document instead: every key survives, the comments and the spacing do not, and the command warns and names the file when a write lands that way. Registration takes effect for the next session started; goose reads its config at startup.
